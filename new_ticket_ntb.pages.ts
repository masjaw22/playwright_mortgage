...
export const newTicket = async (page, record) => {
    ...
    await mainFrame.contentFrame().locator('select[name="maintenanceSBUCode"]').selectOption('02')
    ...
    await mainFrame.contentFrame().locator('input[name="firstIdentificationNumber"]').fill('3218978475673245')
    await mainFrame.contentFrame().getByRole('button', { name: 'Search' }).click()

    await page1.waitForLoadState()
    await page1.waitForTimeout(2000)

    await mainFrame.contentFrame().getByRole('button', { name: 'Proceed' }).click()
    ...
    await mainFrame.contentFrame().locator('input[name="customerVO\\.name"]').fill('ANAS PAING')
    ...
    await mainFrame.contentFrame().locator('input[name="customerVO\\.registeredDateDisplay"]').fill('10/11/1940')
