...
import { new_ticket_etb } from './pages/new_ticket_etb.pages'
...
test.describe('New Ticket Creation ETB', () => {
    test.beforeEach(async ({ page }) => {
        await page.goto('/', {waitUntil: 'commit'}) // Adjust to your actual base URL
    ...
    for (const ticketRecord of ticketRecords) {
        if (ticketRecord.Status === 'Y') {
            test(`Ticket Creation for ETB ${ticketRecord.TestID}`, async ({ page }) => {
                ...
                await test.step(`Create New Ticket ${ticketRecord.TestID}`, async () => {
                    await new_ticket_etb(page, ticketRecord) // Pass context if needed
